...
def check_for_multiple_hits(in_hmm, factor_diff, min_eval, outfile_name):
    ...
    for row_group in rows_to_filter:
        ...
        if (e_vals[0] == 0 and e_vals[1] > 0) or (e_vals[1]/e_vals[0] >= factor_diff): #checks to see if best evalue is small enough compared to next best hit
            rows += [sorted_rows[0]]
...
def find_paralogs(in_hmm, singleton_out):# , paralog_out):
    ...
    with open(in_hmm, 'r') as infile:
        for line in infile:
            if line[0] != '#':
                prot_name = line.split()[0]
                strain = '_'.join(prot_name.split('_')[0:-1])
